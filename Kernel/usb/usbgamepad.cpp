...
void USBGamePadDevice(TUSBGamePadDevice *pThis, TUSBFunction *pDevice) {
    ...
    pThis->m_pReportBuffer = (u8 *) Memory::Allocate(64);
...
static i32 BitGetSigned(void *buffer, u32 offset, u32 length) {
...
static void USBGamePadDeviceDecodeReport(TUSBGamePadDevice *pThis) {
    ...
    i32 lmax = UNDEFINED, lmin = UNDEFINED, pmin = UNDEFINED, pmax = UNDEFINED;
    i32 naxes = 0, nhats = 0;
    u32 id = 0, state = None;

    u8 *pReportBuffer = pThis->m_pReportBuffer;
    i8 *pHIDReportDescriptor = (i8 *) pThis->m_pHIDReportDescriptor;
                ...
                if ((arg & 0x03) == 0x02) {// INPUT(Data,Var)
                    if (state == GamePadAxis) {
                        for (int i = 0; i < count && i < MAX_AXIS; i++) {
                            ...
                            int value = (pState->axes[naxes].minimum < 0) ? BitGetSigned(pReportBuffer, offset + i * size, size) : BitGetUnsigned(pReportBuffer, offset + i * size, size);
...
boolean USBGamePadDeviceConfigure(TUSBFunction *pUSBFunction) {
    ...
    while ((pEndpointDesc = (TUSBEndpointDescriptor *) USBFunctionGetDescriptor(&pThis->m_USBFunction, DESCRIPTOR_ENDPOINT)) != 0) {
        if ((pEndpointDesc->bmAttributes & 0x3F) == 0x03)// Interrupt
        {
            if ((pEndpointDesc->bEndpointAddress & 0x80) == 0x80)// Input
            {
                ...
                pThis->m_pEndpointIn = (TUSBEndpoint *) Memory::Allocate(sizeof(TUSBEndpoint));
            ...
            } else// Output
            {
                ...
                pThis->m_pEndpointOut = (TUSBEndpoint *) Memory::Allocate(sizeof(TUSBEndpoint));
    ...
    pThis->m_pHIDReportDescriptor = (unsigned char *) Memory::Allocate(pHIDDesc->wReportDescriptorLength);
    ...
    if (DWHCIDeviceControlMessage(USBFunctionGetHost(&pThis->m_USBFunction),
                                  ...
                                  pThis->m_pHIDReportDescriptor, pHIDDesc->wReportDescriptorLength) != pHIDDesc->wReportDescriptorLength) {
    ...
    if (pThis->m_State.naxes == 0 && pThis->m_State.nhats == 0 && pThis->m_State.nbuttons == 0) {
        return FALSE;
    }
    ...
    if (USBFunctionGetDevice(pUSBFunction)->m_pDeviceDesc->idVendor == 0x054c && USBFunctionGetDevice(pUSBFunction)->m_pDeviceDesc->idProduct == 0x0268) {
    ...
    DeviceNameService()->AddDevice (StringGet (&DeviceName), pThis, FALSE);
...
void USBGamePadDeviceCompletionRoutine(TUSBRequest *pURB, void *pParam, void *pContext) {
    ...
    if (USBRequestGetStatus(pURB) != 0 && USBRequestGetResultLength(pURB) > 0) {
