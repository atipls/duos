...
void StringPutChar(TString *pThis, char chChar, usize nCount /* = 1 */);
void StringPutString(TString *pThis, const char *pString);
void StringReserveSpace(TString *pThis, usize nSpace);
...
void String2(TString *pThis, const char *pString) {
    ...
    pThis->m_pBuffer = (char *) Memory::Allocate(pThis->m_nSize);
...
void _String(TString *pThis) {
    ...
    if (pThis->m_pBuffer != 0) {
        Memory::Deallocate(pThis->m_pBuffer);
        pThis->m_pBuffer = 0;
    }
}
...
usize StringGetLength(TString *pThis) {
    assert(pThis != 0);
...
void StringAppend(TString *pThis, const char *pString) {
    ...
    char *pBuffer = (char *) Memory::Allocate(pThis->m_nSize);
...
void StringPutChar(TString *pThis, char chChar, usize nCount) {
    assert(pThis != 0);
...
void StringReserveSpace(TString *pThis, usize nSpace) {
    ...
    usize nOffset = pThis->m_pInPtr - pThis->m_pBuffer;
    usize nNewSize = nOffset + nSpace + 1;
    if (pThis->m_nSize >= nNewSize) {
        return;
    }

    nNewSize += FORMAT_RESERVE;
    char *pNewBuffer = (char *) Memory::Allocate(nNewSize);
