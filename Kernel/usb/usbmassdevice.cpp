...
boolean USBBulkOnlyMassStorageDeviceConfigure(TUSBFunction *pUSBFunction) {
    ...
    if (pThis->m_pEndpointIn == 0 || pThis->m_pEndpointOut == 0) {
        USBFunctionConfigurationError(&pThis->m_USBFunction, FromUmsd);
    ...
    DeviceNameService()->AddDevice(StringGet(&DeviceName), pThis, true);
...
int USBBulkOnlyMassStorageDeviceRead(TUSBBulkOnlyMassStorageDevice *pThis, void *pBuffer, unsigned nCount) {
    ...
    } while (nResult != (int) nCount && --nTries > 0);
...
int USBBulkOnlyMassStorageDeviceTryRead(TUSBBulkOnlyMassStorageDevice *pThis, void *pBuffer, unsigned nCount) {
    ...
    if ((pThis->m_ullOffset & UMSD_BLOCK_MASK) != 0 || pThis->m_ullOffset > UMSD_MAX_OFFSET) {
        return -1;
    }
