...
void DWHCITransferStageData(TDWHCITransferStageData *pThis, unsigned nChannel, TUSBRequest *pURB, boolean bIn, boolean bStatusStage) {
    ...
    pThis->m_bSplitTransaction = USBDeviceGetHubAddress(pThis->m_pDevice) != 0 && pThis->m_Speed != USBSpeedHigh;
    ...
    assert(((usize) pThis->m_pBufferPointer & 3) == 0);
    ...
    if (pThis->m_bSplitTransaction) {
        ...
        if (USBDeviceGetHubAddress(pThis->m_pDevice) == 0 && pThis->m_Speed != USBSpeedHigh) {
...
void DWHCITransferStageDataTransactionComplete(TDWHCITransferStageData *pThis, u32 nStatus, u32 nPacketsLeft, u32 nBytesLeft) {
    ...
    if (nStatus & (DWHCI_HOST_CHAN_INT_ERROR_MASK | DWHCI_HOST_CHAN_INT_NAK | DWHCI_HOST_CHAN_INT_NYET)) {
        return;
    }
    ...
    if (pThis->m_bSplitTransaction && pThis->m_bSplitComplete && nBytesTransfered == 0 && pThis->m_nBytesPerTransaction > 0) {
    ...
    if (!pThis->m_bSplitTransaction || pThis->m_bSplitComplete) {
...
boolean DWHCITransferStageDataIsPeriodic(TDWHCITransferStageData *pThis) {
    ...
    return Type == EndpointTypeInterrupt || Type == EndpointTypeIsochronous;
}
...
u32 DWHCITransferStageDataGetDMAAddress(TDWHCITransferStageData *pThis) {
    ...
    return (u32) (usize) pThis->m_pBufferPointer;
}
...
u32 DWHCITransferStageDataGetStatusMask(TDWHCITransferStageData *pThis) {
    assert(pThis != 0);
    u32 nMask = DWHCI_HOST_CHAN_INT_XFER_COMPLETE | DWHCI_HOST_CHAN_INT_HALTED | DWHCI_HOST_CHAN_INT_ERROR_MASK;

    if (pThis->m_bSplitTransaction || DWHCITransferStageDataIsPeriodic(pThis)) {
        nMask |= DWHCI_HOST_CHAN_INT_ACK | DWHCI_HOST_CHAN_INT_NAK | DWHCI_HOST_CHAN_INT_NYET;
    }
