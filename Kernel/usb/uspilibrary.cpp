...
static TUSPiLibrary *s_pLibrary = nullptr;

int USPiInitialize(void) {
    LogWrite(FromUSPi, LOG_DEBUG, "Initializing " USPI_NAME " " USPI_VERSION_STRING);

    s_pLibrary = allocate<TUSPiLibrary>();
    new (&s_pLibrary->NameService) TDeviceNameService();

    DWHCIDevice(&s_pLibrary->DWHCI);
    s_pLibrary->pEth0 = nullptr;
    s_pLibrary->pEth10 = nullptr;
    ...
    if (!DWHCIDeviceInitialize(&s_pLibrary->DWHCI)) {
        ...
        s_pLibrary->NameService.~TDeviceNameService();
        Memory::Deallocate(s_pLibrary);
        s_pLibrary = nullptr;

        return 0;
    }

    s_pLibrary->pUKBD1 = (TUSBKeyboardDevice *) DeviceNameService()->GetDevice("ukbd1", FALSE);
    s_pLibrary->pUMouse1 = (TUSBMouseDevice *) DeviceNameService()->GetDevice("umouse1", FALSE);
    s_pLibrary->pMIDI1 = (TUSBMIDIDevice *) DeviceNameService()->GetDevice("umidi1", FALSE);

    LogWrite(FromUSPi, LOG_DEBUG, "Mouse: %s", s_pLibrary->pUMouse1 != nullptr ? "yes" : "no");
    LogWrite(FromUSPi, LOG_DEBUG, "Keyboard: %s", s_pLibrary->pUKBD1 != nullptr ? "yes" : "no");
    ...
    for (unsigned i = 0; i < MAX_DEVICES; i++) {
        ...
        s_pLibrary->pUMSD[i] = (TUSBBulkOnlyMassStorageDevice *) DeviceNameService()->GetDevice(StringGet(&DeviceName), TRUE);

        _String(&DeviceName);
    }

    s_pLibrary->pEth0 = (TSMSC951xDevice *) DeviceNameService()->GetDevice("eth0", FALSE);
    s_pLibrary->pEth10 = (TLAN7800Device *) DeviceNameService()->GetDevice("eth10", FALSE);
    ...
    for (unsigned i = 0; i < MAX_DEVICES; i++) {
        ...
        s_pLibrary->pUPAD[i] = (TUSBGamePadDevice *) DeviceNameService()->GetDevice(StringGet(&DeviceName), FALSE);
...
int USPiMassStorageDeviceRead(unsigned long long ullOffset, void *pBuffer, unsigned nCount, unsigned nDeviceIndex) {
    ...
    if (nDeviceIndex >= MAX_DEVICES || s_pLibrary->pUMSD[nDeviceIndex] == 0) {
        return -1;
    }
...
const USPiGamePadState *USPiGamePadGetStatus(unsigned nDeviceIndex) {
    ...
    if (nDeviceIndex >= MAX_DEVICES || s_pLibrary->pUPAD[nDeviceIndex] == 0) {
        return 0;
    }
