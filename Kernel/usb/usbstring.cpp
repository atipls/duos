...
void USBString(TUSBString *pThis, struct TUSBDevice *pDevice) {
    ...
    pThis->m_pString = allocate<TString>();
...
void USBStringCopy(TUSBString *pThis, TUSBString *pParent) {
    ...
    if (pParent->m_pUSBString != 0) {
        pThis->m_pUSBString = (TUSBStringDescriptor *) Memory::Allocate(pParent->m_pUSBString->bLength);
    ...
    pThis->m_pString = (TString *) Memory::Allocate(sizeof(TString));
    assert(pThis->m_pString != 0);
...
void _USBString(TUSBString *pThis) {
    ...
    Memory::Deallocate(pThis->m_pString);
    pThis->m_pString = 0;

    if (pThis->m_pUSBString != 0) {
        Memory::Deallocate(pThis->m_pUSBString);
        pThis->m_pUSBString = 0;
    }
...
boolean USBStringGetFromDescriptor(TUSBString *pThis, u8 ucID, u16 usLanguageID) {
    ...
    pThis->m_pUSBString = (TUSBStringDescriptor *) Memory::Allocate(USBSTR_MIN_LENGTH);
    assert(pThis->m_pUSBString != 0);
    ...
    if (ucLength < 2 || (ucLength & 1) != 0 || pThis->m_pUSBString->bDescriptorType != DESCRIPTOR_STRING) {
        return FALSE;
    }

    if (ucLength > USBSTR_MIN_LENGTH) {
        Memory::Deallocate(pThis->m_pUSBString);
        pThis->m_pUSBString = (TUSBStringDescriptor *) Memory::Allocate(ucLength);
        assert(pThis->m_pUSBString != 0);
        ...
        if (pThis->m_pUSBString->bLength != ucLength || (pThis->m_pUSBString->bLength & 1) != 0 || pThis->m_pUSBString->bDescriptorType != DESCRIPTOR_STRING) {
            return FALSE;
        }
    }
    ...
    usize nLength = (pThis->m_pUSBString->bLength - 2) / 2;
...
u16 USBStringGetLanguageID(TUSBString *pThis) {
    ...
    TUSBStringDescriptor *pLanguageIDs = (TUSBStringDescriptor *) Memory::Allocate(USBSTR_MIN_LENGTH);
    ...
    if (DWHCIDeviceGetDescriptor(USBDeviceGetHost(pThis->m_pDevice),
        ...
        Memory::Deallocate(pLanguageIDs);

        return USBSTR_DEFAULT_LANGID;
    }

    u8 ucLength = pLanguageIDs->bLength;
    if (ucLength < 4 || (ucLength & 1) != 0 || pLanguageIDs->bDescriptorType != DESCRIPTOR_STRING) {
        Memory::Deallocate(pLanguageIDs);
    ...
    if (ucLength > USBSTR_MIN_LENGTH) {
        Memory::Deallocate(pLanguageIDs);
        pLanguageIDs = (TUSBStringDescriptor *) Memory::Allocate(ucLength);
        assert(pLanguageIDs != 0);
        ...
        if (pLanguageIDs->bLength != ucLength || (pLanguageIDs->bLength & 1) != 0 || pLanguageIDs->bDescriptorType != DESCRIPTOR_STRING) {
            Memory::Deallocate(pLanguageIDs);
    ...
    usize nLength = (pLanguageIDs->bLength - 2) / 2;
